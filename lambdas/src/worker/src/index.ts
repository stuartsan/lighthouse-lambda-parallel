...
import {createLighthouse} from "./create-lighthouse"
...
async function updateJobItemAndCreateRunItem(
    jobId: string,
    jobAttrToIncrement: string,
    runId: string,
    runUrl: string,
    runMetadata: object,
    runError?: string
) {
    const updatedJob: DocumentClient.UpdateItemInput = {
    ...
    const newRun: DocumentClient.PutItemInput = {
...
const s3Key = (jobId: string, runId: string, outputFormat: string) =>
    `raw_reports/${outputFormat}/jobs/${jobId}/runs/${runId}.${outputFormat}`;

async function doesRunItemAlreadyExist(runId: string, consistentRead = false) {
...
async function uploadReportsToS3(
    jsonReportS3Key: string,
    htmlReportS3Key: string,
    jsonReport: any,
    htmlReport: any
) {
    return Promise.all([
...
export const handler = async (event: any = {}) => {
    ...
    if (record.Sns.TopicArn === process.env.DLQ_ARN) {
        ...
        let jobId: string, metadata: object;
        try {
